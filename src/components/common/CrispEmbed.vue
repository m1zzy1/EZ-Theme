...
export default {
  ...
  setup() {
    ...
    const getCrispWebsiteId = () => {
      ...
      const directId = CUSTOMER_SERVICE_CONFIG?.crispWebsiteId?.trim();
      ...
      const crispIdMatch = CUSTOMER_SERVICE_CONFIG?.customHtml?.match(/CRISP_WEBSITE_ID\s*=\s*["']([^"']*)["']/);

      return crispIdMatch ? crispIdMatch[1].trim() : '';
    ...
    const initCrisp = async () => {
      ...
      try {
        ...
        if (!websiteId) {
          ...
          console.error('未配置Crisp网站ID，请在CUSTOMER_SERVICE_CONFIG.crispWebsiteId中填写');
